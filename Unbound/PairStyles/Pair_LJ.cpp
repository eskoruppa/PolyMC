...
Pair_LJ::Pair_LJ( Chain * ch,
            TypeGroup * type_group_A,
            TypeGroup * type_group_B,
            double cutoff,
            std::vector<double> params) :
Pair(ch,type_group_A,type_group_B,cutoff,params)
{
    std::cout << " -> Pair Style 'Lennard Jones' initialized for atom types " << type_group_A->id << " and " << type_group_B->id << std::endl;

    if (params.size() < 2) {
        std::cout << "Error in Pair_LJ::Pair_LJ: parameters for Lennard Jones pair style require 2 parameters. " << params.size() << " given." << std::endl;
        std::exit(0);
    }
    epsilon = params[0];
    rmin    = params[1];
...
double Pair_LJ::Eval_change_type(const arma::mat * pos) {
...
double Pair_LJ::Eval_Energy(const arma::mat * pos)
{
    double E,dist;
    arma::colvec pos_i;
...
double Pair_LJ::Eval_Delta_Energy( const arma::mat * pos_new,
                                const arma::mat * pos_old)
{
//    std::cout << "Evaluating Delta E - Lennard Jones" << std::endl;
...
double Pair_LJ::Potential(double distance) {
    double frac = rmin/distance;
    frac = frac*frac*frac*frac*frac*frac;


//    std::cout << epsilon * ( frac*frac - 2*frac  ) << std::endl;

    return epsilon * ( frac*frac - 2*frac  );
}
