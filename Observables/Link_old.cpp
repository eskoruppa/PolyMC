...
double Chain::cal_langowski_writhe_1a() {
    ...
    for (unsigned i=1;i<num_bp-1;i++) {
        ...
        for (int j=0;j<i-1;j++) {
            ...
            Omega = Omega * sgn(dot(cross(r34,r12),r13));
            if (Omega > 1) {
                std::cout << "##################" << std::endl;
                std::cout << i << " " << j << std::endl;
                std::cout << Omega/(2*M_PI) << std::endl;
                std::cout << asin(dot(n1,n2)) << std::endl;
                std::cout << asin(dot(n2,n3)) << std::endl;
                std::cout << asin(dot(n3,n4)) << std::endl;
                std::cout << asin(dot(n4,n1)) << std::endl;
            }
            if (Omega == Omega) {
                Wr = Wr + Omega;
            }
...
double Chain::cal_langowski_writhe_1a(double density) {
    ...
    if (closed_topology == true) {
        ...
        for (int j=1;j<(num_points-1)-1;j++) {
            ...
            //~ std::cout << "-----------------\n";
            //~ std::cout << asin(dot(n1,n2)) << "\n";
            //~ std::cout << asin(dot(n2,n3)) << "\n";
            //~ std::cout << asin(dot(n3,n4)) << "\n";
            //~ std::cout << asin(dot(n4,n1)) << "\n";
...
double Chain::cal_langowski_writhe_1a(const arma::mat& pos, bool closed) {
    ...
    for (int i=1;i<segs-1;i++) {
        ...
        for (int j=0;j<i-1;j++) {
            ...
            Omega = std::asin(arma::dot(n1,n2)) + std::asin(arma::dot(n2,n3)) + std::asin(arma::dot(n3,n4)) + std::asin(arma::dot(n4,n1));
            Omega = Omega * sgn(arma::dot(arma::cross(r34,r12),r13));
            if (Omega == Omega) {
                Wr = Wr + Omega;
            }
        }
