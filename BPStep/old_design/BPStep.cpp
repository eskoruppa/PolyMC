...
: id(id_in_chain), closed(closed), trial_pending(false), disc_len(disc_len), T(temp), T0_subtract(false)
{
...
//        eval_energy_right.push_back( select_EvalEnergy(idbu->methods[coup_right+i+1],idbu->interactions[coup_right+i+1],false) );
    ...
    */
    ...
    local_isotropic = (coupling_range==0 && T0_store(0) == T0_store(1) && eval_energy_diag->isotropic_bending());
